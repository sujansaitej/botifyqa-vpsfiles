...
from excel_test2 import run_steps_from_excel
...
async def read_from_excel():
    await run_steps_from_excel("demo.xlsx")

    return (
        "",                                        # errors_output
        gr.update(value="Ran excel code", interactive=False),  # stop_button
...
async def run_with_stream(
    ...
    # convert task from Gradio element to string
    task = str(task)

    # split task into steps by newline
    tasks = task.split("\n")
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("🤖 Run Agent", id=4):
                ...
                with gr.Row():
                    ...
                    stop_button = gr.Button("⏹️ Stop (this one)", variant="stop", scale=1)
